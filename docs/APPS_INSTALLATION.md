...
2. **Multiple apps with different branches:**
   ```bash
   CUSTOM_APPS=myorg/app1:production,myorg/app2:main,myorg/app3:version-14
   ```

3. **Apps with default branch (production):**
   ```bash
   CUSTOM_APPS=myorg/app1,myorg/app2:main,myorg/app3
   ```
   In this case, `app1` and `app3` will use the `production` branch by default.
...
CUSTOM_APPS=mycompany/custom_erp:main,mycompany/reports_app:production,mycompany/integrations:version-14
...
3. **Branch defaults**: If no branch is specified, `production` is used
...
CUSTOM_APP_BRANCH=production
CUSTOM_APP_NAME=myapp
```

**New:**
```bash
CUSTOM_APPS=myorg/myapp:production
```
